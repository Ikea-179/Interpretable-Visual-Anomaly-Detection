...
def main():
    ...
    # train with nu
    #X_train = np.zeros((len(train_dataset),3,256,256))
    #Y_train = np.zeros((len(train_dataset),1))
    #X_test = np.zeros((len(test_dataset),3,256,256))
    ...
    for nu in nus:
        ...
        for j in range(test_rep_count):
            ...
            roc_auc, prc_auc = calc_metrics(Y_test, Y_predict)
