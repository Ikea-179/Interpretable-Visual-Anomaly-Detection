...
def main():
    ...
    for batch_idx, (data, gt,label) in enumerate(train_loader):
        X_train =data.view(-1,1).detach().numpy()
        Y_train = gt.view(-1,1).detach().numpy()
    for batch_idx, (data, gt,label) in enumerate(test_loader):
        X_test = data.view(-1,1).detach().numpy()
        Y_test = gt.view(-1,1).detach().numpy()
    ...
    for nu in nus:
        ...
        for j in range(test_rep_count):
            ...
            roc_auc, prc_auc = calc_metrics(Y_test, scores)
