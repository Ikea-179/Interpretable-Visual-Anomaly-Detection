...
def main():
    ...
    parser.add_argument('--layer', type=str, default='encoder.15',
    ...
    checkpoint = torch.load(args.model_path+'_'+'ssim_'+args.layer+'_'+args.loss+'_'+str(args.one_class)+'ssim_'+args.ssim+'_unet.pth')
    ...
    #recon_sum = torch.zeros(1,3,256,256).to(device)
    z1_sum = torch.zeros(1,3,256,256).to(device)
    z2_sum = torch.zeros(1,3,256,256).to(device)
    z3_sum = torch.zeros(1,3,256,256).to(device)
    z4_sum = torch.zeros(1,3,256,256).to(device)
    z5_sum = torch.zeros(1,3,256,256).to(device)
    ...
    with torch.no_grad():
        for batch_idx,(data,_)in enumerate(train_loader):
            ...
            z5_sum += z5
            mu_sum += data  
        z1_mean = z1_sum/len(train_loader.dataset)
        z2_mean = z2_sum/len(train_loader.dataset)
        z3_mean = z3_sum/len(train_loader.dataset)
        z4_mean = z4_sum/len(train_loader.dataset)
        z5_mean = z5_sum/len(train_loader.dataset)       
        mu_mean = mu_sum/len(train_loader.dataset)
        #for batch_idx,(data,_)in enumerate(train_loader):
            #data = data.to(device)
            #recon_batch, x = model(data-mu_mean)
            #recon_sum += recon_batch
        #recon_mean = recon_sum/len(train_loader.dataset)  
    ...
    test_index=0
    gcam = GradCAM(model1, target_layer=args.layer, cuda=True) 
    for batch_idx, (data, y) in enumerate(test_loader):
        model1.eval()
        data = data.to(device)
        norm_mu = torch.exp(torch.pow(mu_mean-data,3))
        ...
        recon_batch, z1,z2,z3,z4,z5 = gcam.forward(norm_mu)
        #recon_batch1, x1 = gcam.forward(recon_mean)
        gcam.backward(z3)
        #recon_batch, x = gcam.forward(data-mu_mean)
        #_, s = gcam.forward(data-recon_batch)
        #gcam.backward(s)
        ...
        for i in range(data.size(0)):
            ...
            im_path = args.result_dir+'_'+args.layer+'_'+args.loss+'_'+args.test+'ssim_'+args.ssim+'_unet'
