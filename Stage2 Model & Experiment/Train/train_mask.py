...
from model import ConvVAE, ConvAutoencoder, ConvAE_mvtec
...
def train(epoch, model, train_loader, optimizerR, optimizerG, args, schedulerR, schedulerG, mu_mean, ssim):
    ...
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        recon_batch, _ = model(data-mu_mean)
        ...
        if B == args.batch_size:
            ...
            recon_batch, x = gcam.forward(data-mu_mean)
            gcam.backward(x)
...
def test(epoch, model, test_loader, args,mu_mean, ssim):
    ...
    for batch_idx, (data, _) in enumerate(test_loader):
        ...
        test_loss += loss2.item()+10*loss1.item()
...
def save_checkpoint(state, is_best, outdir,args):
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    checkpoint_file = os.path.join(outdir, 'ae_mvtec_checkpoint'+'_'+args.layer+'_'+args.loss+'_'+str(args.one_class)+'ssim_'+args.ssim+'_unet'+'.pth')
    best_file = os.path.join(outdir, 'ae_mvtec_model_best_ssim'+'_'+args.layer+'_'+args.loss+'_'+str(args.one_class)+'ssim_'+args.ssim+'_unet'+'.pth')
...
def main():
    ...
    parser.add_argument('--result_dir', type=str, default='./train_mvtec', metavar='DIR',
    ...
    parser.add_argument('--layer', type=str, default='down5.conv1',
    ...
    mu_sum = torch.zeros(1,3,256,256).to(device)
    ...
    optimizerG = optim.Adam(model.parameters(), lr=1e-2)
    schedulerR = torch.optim.lr_scheduler.OneCycleLR(optimizerR, max_lr=0.0001, div_factor = 100, total_steps=args.epochs*args.batch_size*8)#
    schedulerG = torch.optim.lr_scheduler.OneCycleLR(optimizerG, max_lr=0.0001, div_factor = 100, total_steps=args.epochs*args.batch_size)
    lrs = []
    steps = []
    start_epoch = 0
    ...
    if args.resume:
        if os.path.isfile(args.resume+'_'+args.layer+'_'+args.loss+'_'+str(args.one_class)+'ssim_'+args.ssim+'_unet'+'.pth'):
            print('=> loading checkpoint %s' % args.resume+'_'+args.layer+'_'+args.loss+'_'+str(args.one_class)+'ssim_'+args.ssim+'_unet'+'.pth')
            checkpoint = torch.load(args.resume+'_'+args.layer+'_'+args.loss+'_'+str(args.one_class)+'ssim_'+args.ssim+'_unet'+'.pth')
    ...
    for epoch in range(start_epoch, start_epoch+args.epochs):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (data, _) in enumerate(test_loader):
                ...
                save_dir = os.path.join('./',args.result_dir+'_'+args.layer+'_'+args.loss+'_'+str(args.one_class)+'ssim_'+args.ssim+'_unet')
                if not os.path.exists(save_dir):
                    os.makedirs(save_dir)
                save_image(data[0].view(1, 3, 256, 256), os.path.join(save_dir, str(epoch) +'_origin_sample' + '.png'))
                save_image(recon_batch[0].view(1, 3, 256, 256), os.path.join(save_dir, str(epoch) + '_recon_sample' + '.png'))
